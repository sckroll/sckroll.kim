...
- 후보 속성: slug, 공개 상태, 게시 일시, 요약, 태그, 대표 이미지, 대표 노출 여부, 외부 URL
- 속성명·타입·필수 여부·기본값의 사용자 검수 필요

### PROJECTS

- 후보 속성: slug, 공개 상태, 대표 노출 여부 또는 정렬값, 역할, 성과·지표, 대표 이미지, 서비스 URL, 저장소 URL, 공개용 맥락
- 기존 프로젝트 상태와 별도 공개 상태의 필요
- 기존 40개 행의 공개 기준과 마이그레이션 방식 검수 필요
